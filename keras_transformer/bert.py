...
class BatchGeneratorForBERT:
    ...
    def generate_batches(self):
        """
        tferas-compatible generator of batches for BERT (can be used with
        ...
        `inputs` is a list of two values:
            1. masked_sequence: an integer tensor shaped as
               ...
               the input sequence, with some words masked by the [MAStf] token.
...
class MaskedPenalizedSparseCategoricalCrossentropy:
    ...
    def __call__(self, y_true, y_pred):
        ...
        num_items_masked = tf.sum(mask, axis=-1) + 1e-6
        masked_cross_entropy = (
            tf.sum(mask * tf.sparse_categorical_crossentropy(y_true_val, y_pred),
                  axis=-1)
            / num_items_masked)
        masked_entropy = (
            tf.sum(mask * -tf.sum(y_pred * tf.log(y_pred), axis=-1), axis=-1)
            / num_items_masked)
