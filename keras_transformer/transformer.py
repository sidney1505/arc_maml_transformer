...
class TransformerTransition(Layer):
    ...
    def __init__(self, activation: Union[str, Callable],
        ...
        :param kwargs: tferas-specific layer arguments.
        """
    ...
    def call(self, inputs, **kwargs):
        ...
        step1 = self.activation(
            K.bias_add(
                K.dot(tf.reshape(inputs, (-1, d_model)),
                      self.weights1),
                self.biases1,
        ...
        step2 = K.bias_add(
            ...
            self.biases2,
            data_format='channels_last')
        new_shape = tf.concat([[-1], input_shape[-2:]], axis=0)
        result = tf.reshape(step2, new_shape)
        return result
...
class TransformerACT(Layer):
    ...
    def __init__(self, halt_epsilon=0.01, time_penalty=0.01, **kwargs):
        ...
        :param kwargs: Any standard parameters for a layer in tferas (like name)
        """
    ...
    def build(self, input_shape):
        ...
        self.halting_biases = self.add_weight(
            ...
            trainable=True)
        self.time_penalty_t = tf.constant(self.time_penalty, dtype=tf.float32)
        return super().build(input_shape)
    ...
    def initialize_control_tensors(self, halting):
        ...
        shapes).
        """
        self.zeros_like_halting = tf.zeros_like(
            halting, name='zeros_like_halting')
        self.ones_like_halting = tf.ones_like(
    ...
    def call(self, inputs, **kwargs):
        ...
        if self.zeros_like_input is None:
            self.zeros_like_input = tf.zeros_like(
        ...
        step_weighted_output = tf.switch(
            any_step_is_active,
            tf.expand_dims(halting_prob, -1) * inputs,
